...
# lancement des tests
preload
echo "$FUNCNAME doit renvoyer des numéros de comptes si parse "${FICHIERDEST}" passé en param normal $(getallnumcompte_withfilename "${FICHIERDEST}")"
...
echo "$FUNCNAME doit renvoyer des numéros de compte si parse "${FICHIERDEST}" passé en param < $(getallnumcompte_withoutfilename  <"${FICHIERDEST}")"
...
function fail_cptpasbq(){
	...
	RESULTAT="$(err_sinumcompte_pasbq ""${FICHIERDEST}"")"; ERR="$?"
...
## verif que bon champ
FONCTION="$FUNCNAME"
# un fichier de type "${FICHIERDEST}" qui jouera le role d'env de test
function preload(){
cat << limitedefichier > temp
date:util:numcompte:nomcompte:sensope:montantope:libope
2011-09-05:co:512:"Banque":D:15138.55:"ouverture compte"
...
#echo -e "$FUNCNAME en appel param normal doit renvoiyer les montants d'un fichier de type "${FICHIERDEST}" \n $(COMMANDE='getmontantope -f temp'; echo "COMMANDE: $COMMANDE"; eval $COMMANDE)"
incremente_compteur
...
PREMIER="$(cat "${FICHIERDEST}" |filtre_getlinesfromuser "co" |wc -l)"
DEUXIEME="$(cat "${FICHIERDEST}" |getallusers |grep "co" |wc -l)"
...
ERR_PAS_TRIE="1"
TRI_OK="0"
echo "$FUNCNAME : test du test: si ens trié, un ensemble trie doit donner un msg err de val ${TRI_OK} $(CMD=$(triparmontant "${FICHIERDEST}" | getmontantope |(LANG=C sort --check --numeric-sort)); ERR=$? ; affiche_test ${ERR} ${TRI_OK} )"

echo "$FUNCNAME : test du test: si ens non trié, un ensemble non trie doit donner un msg err de val ${ERR_PAS_TRIE} $(CMD=$(getmontantope "${FICHIERDEST}" |(LANG=C sort --check --numeric-sort 2>sort.err)); ERR=$? ; affiche_test ${ERR} ${ERR_PAS_TRIE})"
}
# ok
...
PREMIER="$(cat "${FICHIERDEST}"|getallusers |wc -l)"
DEUXIEME="$(cat "${FICHIERDEST}" |getallusers |grep --only-matching -E 'co|pu' |wc -l)"
...
# on sort un format du type pre"${FICHIERDEST}" 
# c a d en ligne tq: 
