...
###############################################################
# 2. depuis "${FICHIERDEST}" multi util multi comptes 
# produire un fichier "${FICHIERDEST}"|util | 512 
############################################################
# journalmultiu_to_extraitgl_monou_monoc
# glmultiu_to_journal_unseulu -f "${FICHIERDEST}" -u co -cpte 512
# construire progressivement la regex multi critères
# seulement 512: gawk -V util=$UTIL -V compte=$COMPTE....  
# $3 -> numcomptes
# $2 -> users
# /$2==util && $3==compte/
# gawk -F: -v util="co" -v compte="512" "\$2==util && \$3==compte" "${FICHIERDEST}"
# gawk -F: -v util=qfd -v compte="dqfqsdf" "$CHAINE" "${FICHIERDEST}"
# gawk -F: $CHAINE
...
echo "prend un fichier de type ofx. renvoie un fichier de type "${FICHIERDEST}""
}
aumoinsun "$@"
...
# test du concept
# cat "${FICHIERDEST}" |getlinesfromuser "co" |getonlybanq 
# getmontantope    := getmontants  #TODO: trier !!!
# => getmontants "${FICHIERDEST}"
